...
inline void init()
{
    ...
    if (id < 0) {
        qWarning("IconFont: failed to load font from %s",
                 qPrintable(fontPath));
    } else {
        qDebug() << "IconFont loaded:" << QFontDatabase::applicationFontFamilies(id);
    }
}
...
inline QIcon icon(char32_t  codepoint,
                  int        size  = 20,
...
inline QIcon iconDual(char32_t codepoint,
                      int      size        = 20,
