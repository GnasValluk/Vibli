...
void PlaylistManager::addTracks(const QList<Track> &tracks)
{
    ...
    if (m_currentIndex < 0 && !m_tracks.isEmpty()) jumpTo(0);
}
...
PlaylistManager::RepeatMode PlaylistManager::repeatMode() const { return m_repeatMode; }
...
int PlaylistManager::nextIndex() const
{
    ...
    if (m_repeatMode == RepeatMode::One) return m_currentIndex;
