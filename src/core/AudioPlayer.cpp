...
AudioPlayer::AudioPlayer(QObject *parent)
  ...
  connect(m_player, &QMediaPlayer::hasVideoChanged, this,
          &AudioPlayer::videoAvailableChanged);
...
void AudioPlayer::seek(qint64 positionMs) { m_player->setPosition(positionMs); }

void AudioPlayer::setVolume(float volume) {
  m_audioOutput->setVolume(qBound(0.0f, volume, 1.0f));
}

void AudioPlayer::setMuted(bool muted) { m_audioOutput->setMuted(muted); }
